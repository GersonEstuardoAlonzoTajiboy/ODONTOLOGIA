...
const CreationBudget = () => {
  ...
  const [treatments, setTreatments] = useState([{ treatment: '', cost: '' }]);
  ...
  const addTreatmentRow = () => {
    setTreatments([...treatments, { treatment: '', cost: '' }]);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        alert('Presupuesto guardados exitosamente.');
        navigate('/patients');
      } else {
